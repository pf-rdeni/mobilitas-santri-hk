...
<div class="mb-3">
    ...
    <span class="text-muted mx-1">/</span>
    <span class="text-dark">Edit Data</span>
</div>

<div class="row">
    <!-- COL 1: Data Pokok (Single Row) -->
    <div class="col-lg-12">
        <form action="<?= base_url('orangtua/santri/update/' . $santri->id) ?>" method="post">
            <?= csrf_field() ?>
            <div class="card card-warning card-outline mb-4 collapsed-card shadow-sm">
                <div class="card-header bg-warning d-flex justify-content-between align-items-center">
                    <h3 class="card-title text-dark font-weight-bold"><i class="fas fa-user-edit mr-2"></i> Edit Data Pokok Santri</h3>
                    <div class="card-tools ml-auto">
                        <button type="button" class="btn btn-tool text-dark pt-0" data-card-widget="collapse" title="Buka/Tutup Data Pokok">
                ...
                <div class="card-body">
                    <div class="row mb-4">
                        <div class="col-md-4 text-center mb-3 mb-md-0">
                            ...
                            <div id="image-preview-container" class="mx-auto bg-light border rounded-circle d-flex align-items-center justify-content-center shadow-sm" style="width: 120px; height: 120px; overflow: hidden; cursor: pointer; border: 3px solid #ffc107 !important;">
                            ...
                            <button type="button" class="btn btn-outline-warning btn-xs mt-2 text-dark" onclick="document.getElementById('foto_input').click()">
                        ...
                        <div class="col-md-8 pt-md-2">
                            <div class="form-group mb-0">
                                ...
                                <input type="text" name="nama" class="form-control form-control-lg" value="<?= old('nama', $santri->nama) ?>" required>
                                <small class="text-muted mt-1 d-block"><i class="fas fa-info-circle mr-1"></i> Klik area lingkaran atau tombol "Ganti Foto" untuk mengubah foto profil.</small>
                            </div>
                        </div>
                    </div>

                    <h5 class="border-bottom pb-2 mb-3 text-warning"><i class="fas fa-info-circle"></i> Detail Personal Santri</h5>
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-12 form-group text-dark">
                ...
                <div class="card-footer text-right">
                    <button type="submit" class="btn btn-warning text-dark"><i class="fas fa-save"></i> Update Data Pokok</button>
                </div>
            </form>
        </div>
        ...
        <div class="card card-info card-outline shadow-sm">
            <div class="card-header d-flex justify-content-between align-items-center bg-info text-white">
                <h3 class="card-title m-0"><i class="fas fa-history mr-2"></i> Riwayat Akademik</h3>
                <button type="button" class="btn btn-light btn-sm text-info ml-auto font-weight-bold" data-toggle="modal" data-target="#modalRiwayat">
                        ...
                        <table class="table table-striped table-bordered m-0">
                            ...
                            <tbody>
                                ...
                                <tr class="small">
                                    ...
                                    <td class="align-middle"><?= esc($r->kelas) ?></td>
                                    <td class="align-middle"><?= esc($r->asrama) ?></td>
                                    <td class="text-center align-middle">
                                        <form action="<?= base_url('orangtua/santri/riwayat/delete/' . $r->id) ?>" method="post" onsubmit="return confirm('Hapus riwayat ini?');">
                                            <?= csrf_field() ?>
            ...
            <div class="card-footer bg-light text-muted small border-top">
                <i class="fas fa-exclamation-triangle mr-1 text-warning"></i> Pastikan untuk selalu memperbarui riwayat kelas & asrama santri setiap ada kenaikan jenjang.
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title"><i class="fas fa-plus-circle mr-1"></i> Tambah Riwayat Akademik</h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="<?= base_url('orangtua/santri/riwayat') ?>" method="post">
                <?= csrf_field() ?>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="kelas" required placeholder="Contoh: 7A, 10B">
                    </div>
                    <div class="form-group">
                        <label>Asrama / Rayon <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" name="asrama" required placeholder="Contoh: Asrama Khadijah">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                    <button type="submit" class="btn btn-info">Simpan Riwayat</button>
                </div>
            </form>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if(initProvId) {
                // If we have province, load kabupaten
                loadKabupaten(initProvId, initKabId);
            }
        }
    });

    // Helper functions for loading data to keep it clean
